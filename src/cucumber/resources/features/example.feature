Feature: Cucumber Tests
  ...
  Scenario Outline: Should return 200 Unauthorized when contacting an external API
    ...
    When a <internal _http_method> request is sent to <endpoint> with a request body of <request_body> with headers <headers>
    Then a <internal_response_code> response is returned
    And a <external_http_method> request was made to the external server
    Examples:
      | internal _http_method | external_http_method | endpoint          | request_body | headers   | external_response_code | internal_response_code |
